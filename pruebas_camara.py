...
relacion_pixel_cm = 5/166  # Relación de 5 cm por 166 px
...
window_name = "Detección de Círculos con Mediciones"
...
while True:
    ...
    cv2.putText(output, "(0,0)", (center_x + 15, center_y), 
    ...
    if circles is not None:
        ...
        for circle in circles[0, :]:
            ...
            dist_x = x - center_x
            dist_y =  center_y- y# Y invertido (positivo hacia arriba)
            ...
            # Mostrar las coordenadas relativas al centro
            coord_text = f"({dist_x},{dist_y})"
            cv2.putText(output, coord_text, (x + 10, y - 10), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            # Mostrar la distancia en X
            x_dist_text = f"X: {dist_x} px"
            x_text_pos_y = y + 20 if y < height - 30 else y - 20  # Evitar texto fuera de la imagen
            cv2.putText(output, x_dist_text, (center_x + dist_x//2, x_text_pos_y), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            
            # Mostrar la distancia en Y
            y_dist_text = f"Y: {dist_y} px"
            y_text_pos_x = x + 10 if x < width - 100 else x - 100  # Evitar texto fuera de la imagen
            cv2.putText(output, y_dist_text, (y_text_pos_x, center_y + dist_y//2), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            
            # Imprimir valores para depuración
            print(f"Centro de imagen: ({center_x}, {center_y}), Centro de círculo: ({x}, {y}), Distancia: ({dist_x}, {dist_y})")
